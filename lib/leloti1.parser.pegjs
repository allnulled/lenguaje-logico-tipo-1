      ...
      if(suceso.tipo === "suceso puro") {
        ...
        suceso.concatenaciones.forEach((concatenacion, indiceConcatenacion) => {
          if(indiceConcatenacion !== 0)  {
            ...
            const estaResueltaConcatenacion = typeof estaResueltaConcatenacion === "boolean";
...
sentencia_porque = consecuencia:suceso_grupal ( " porque " / " en tanto que " ) causa:suceso "."  {return {tipo:"causalidad",causa,consecuencia}}
...
negacion = "no "
suceso_puro = "{" [^\}]+ "}" {return {tipo:"suceso puro",texto:text()}}
EOL = "\n" / "\r\n"
EOS = (EOL* / !.)