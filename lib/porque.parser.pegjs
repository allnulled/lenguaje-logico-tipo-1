...
sentencia_pregunta_por_que = op:( "¿por qué" _ ) suceso:suceso_no_agrupado "?" {return {tipo:"pregunta",suceso, operador: op.trim() + "?"}}
sentencia_pregunta = op:( "¿" _? ) suceso:suceso _? "?" {return {tipo:"pregunta",suceso, operador: op.trim() + "?"}}
...
suceso_concatenacion = op:( _ ("y" / "o") _ ) suceso:suceso {return {operador:op.trim(), ...suceso}}
