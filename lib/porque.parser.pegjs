    ...
    cuestionar(sentencia) {
      ...
      if(operador === "¿?") {
        ...
        if(typeof resultadoSuceso === "boolean") {
          ...
          return `No se puede afirmar que ${suceso.texto} porque está por determinar.`
        }
      }
      return operador;
    }
...
sentencia_pregunta_por_que = op:( "¿por qué " ) suceso:suceso_no_agrupado "?" {return {tipo:"pregunta",suceso, operador: op.trim() + "?"}}
